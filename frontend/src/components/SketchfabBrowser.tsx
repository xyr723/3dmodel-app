...
export function SketchfabBrowser({ onModelSelect }: SketchfabBrowserProps) {
  const addSearchRecord = useSearchStore((state) => state.addSearchRecord);
  ...
  const handleSearch = async () => {
    ...
    try {
      if (searchQuery.trim()) {
        ...
        // 保存搜索记录
        const searchRecord: SketchfabSearchRecord = {
          id: Date.now().toString() + Math.random().toString(36).substr(2, 9),
          query: searchQuery,
          category: selectedCategory || undefined,
          license: selectedLicense || undefined,
          total_count: response.total_count,
          models_found: response.models.length,
        ...
        await addSearchRecord(searchRecord);
        // 自动选择第一个模型进行预览
        ...
        // 保存浏览热门模型的记录
        const browseRecord: SketchfabSearchRecord = {
          id: Date.now().toString() + Math.random().toString(36).substr(2, 9),
          query: "热门推荐" + (selectedCategory ? ` - ${selectedCategory}` : ""),
          category: selectedCategory || undefined,
          license: undefined,
          total_count: popular.length,
          models_found: popular.length,
          search_time: 0,
          timestamp: Date.now(),
          filters_applied: {
            downloadable: filters.downloadable,
        ...
        await addSearchRecord(browseRecord);
        // 自动选择第一个模型进行预览
        if (popular.length > 0) {
