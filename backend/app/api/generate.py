...
# 依赖注入（改为单例，确保任务状态可跨请求持久）
_model_service_singleton: Optional[ModelService] = None
_cache_service_singleton: Optional[CacheService] = None
...
async def generate_3d_model(
	...
	Args:
		...
		cache_service: 缓存服务
		api_key: API密钥
		provider: 可选，覆盖后端默认提供商（meshy/local/sketchfab）
	...
	try:
		...
		# 检查缓存（provider也作为缓存键的一部分）
		cache_key = cache_service.generate_cache_key(request) + (f":{(provider or '').lower()}" if provider else "")
		...
		# 调用模型服务生成/检索3D模型
		result = await model_service.generate_model(request, provider_override=provider)
